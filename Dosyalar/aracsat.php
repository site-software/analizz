...
<div class="alert alert-secondary" role="alert"><strong>Önəmli məlumat:</strong> İlk olaraq elana əlavə etmək istədiyiniz şəkilləri seçin və yükləyə basın. Şəkillər görünürsə əgər onda aşağıda yerləşən formu doldura bilərsiniz.</div>
</div>

<div class="col-lg-12" style="padding: 10px;">
  <!-- çoklu foto -->
    <form method="post" id="uploadForm" enctype="multipart/form-data">
        <label>Elan şəkillərini seç</label><br />
        <input type="file" name="resim[]" id="resimler" class="form-control" multiple >
        <input type="submit" name="gorselgonder" value="Yükləmək"/>
    </form>
 
    <!-- loading resmini göster gizle -->
    <div id="upladDurum"></div>
 
    <!-- yüklenen resimleri göster --> 
    <div class="row" id="resimGaleri"></div>
<br />
<!--çoklu foto -->
</div>
</div>
...
<?php
require('db.php');
if(isset($_POST['gorselgonder'])){
    // yuklenecek dizin ve dosya uzantıları
    $hedefDizin = "yuklenen/";
    $dosyaTuru = array('jpg','png','jpeg','gif','rar','zip');
    
    $resimDizi = array();
    foreach($_FILES['resim']['name'] as $key=>$val){
        $_name = md5($_FILES['resim']['name'][$key]);
        $_tmp_name   = $_FILES['resim']['tmp_name'][$key];
        $_size       = $_FILES['resim']['size'][$key];
        $_type       = $_FILES['resim']['type'][$key];
        $_error      = $_FILES['resim']['error'][$key];
        
        // hedef yolu belirle
        $dosyaAdi = basename($_FILES['resim']['name'][$key]);
        $hedefYol = $hedefDizin . $dosyaAdi;
        
        // Dosya uzantısını kontrol et
        $dosyaUzanti = pathinfo($hedefYol,PATHINFO_EXTENSION);
        if(in_array($dosyaUzanti, $dosyaTuru)){    
            // Sunucuya kaydet
            if(move_uploaded_file($_FILES['resim']['tmp_name'][$key],$hedefYol)){
                $resimDizi[] = $hedefYol;
            }
        }
    }
    
    // Galeride gösterilecek kısmı oluştur. not:bootstrap kullanıldı
    if(!empty($resimDizi)){ ?>
        <div class="row">
          <div class="col-lg-12" style="padding: 10px; background: green; color: #FFF; border-radius: 5px;">Resimler başarıyla yüklendi, ilan formunu doldurabilirsiniz</div>
    <?php foreach($resimDizi as $resim_src){ ?>
        <div class="col-md-3" style="padding: 10px">
            <img src="<?php echo $resim_src; ?>" class="img-fluid" alt="">
            <input class="form-control" name="resim[]" value="<?php echo $resim_src; ?>" hidden>
        </div><br />
    <?php } ?>
</div>
<?php }
...
<div class="col-lg-12" style="padding: 10px;display: none;">
...
<div class="mb-3" style="padding: 15px; border: 1px dashed #CCC; border-radius: 3px;">
  <input class="form-control" type="file" id="formFile" name="" accept="image/*">
</div>
</div> 
...
$resim = json_encode($vitrins);
 ...
 $gunce -> execute(array($kullanici,$marka,$yanacaq,$model,$oturucu,$bannovu,$suretler,$yurus,$ilyil,$rengi,$muherrikin,$parasi,$parasitl,$mgucu,$sahip,$vaziyeti,$yersayisi,$aasec,$ilanaciklama,$altsecenek,$tarih,$resim,"0","0"));
