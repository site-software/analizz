...
<head>
  ...
  <title>Nəticələr - <?php echo $ek['siteadi']; ?></title>
...
$cek = $db->query("select * from plakalar where nomresec like '%$nomresec%' or nomresecc like '%$nomresecc%' or nomreseccc like '%$nomreseccc%' or nomresecyaz like '%$nomresecyaz%' ",PDO::FETCH_ASSOC);
$cek = $db->query("select * from plakalar where parasi like '%$fiyat1%' or parasi like '%$fiyat2%' ",PDO::FETCH_ASSOC);

if($cek->rowCount()){
foreach($cek as $plaka){
echo '

<div class="col-md-3">
<div class="card" style="margin-bottom: 15px;">
<a href="plaka.php?id='.$plaka['id'].'">


<div class="plaks">
'.$plaka['nomresec'].'-'.$plaka['nomresecc'].''.$plaka['nomreseccc'].'-'.$plaka['nomresecyaz'].'
</div>

  <div class="card-body">
    <h5 class="card-title" style="text-align: center;color: var(--site-rengi2);">'.$plaka['parasi'].' <sup>'.$plaka['parasitl'].'</sup></h5>
  </div>
</a> 
</div>
</div>
